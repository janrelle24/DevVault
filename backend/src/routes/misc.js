...
router.post('/tags', requireAuth, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        if (err.code === '23505') return res.status(409).json({ error: 'This tag already exists.' });
        console.error(err);
...
router.get('/recently-viewed', requireAuth, async (req, res) => {
    try {
        const result = await db.query(
        `SELECT d.id, d.title, d.slug, d.icon, d.description, rv.viewed_at
        ...
        console.error(err);
        res.status(500).json({ error: 'Could not load recently viewed documents.' });
    }
});
