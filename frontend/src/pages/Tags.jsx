...
export default function Tags() {
    ...
    return (
        <div className="px-6 lg:px-10 py-8 max-w-5xl">
            ...
            <h1 className="text-3xl font-extrabold text-vault-text tracking-tight mb-6">Tags</h1>
        
            <div className="flex flex-wrap gap-2 mb-8">
                <button
                onClick={() => setParams({})}
                className={`text-xs font-medium rounded-md border px-2.5 py-1 transition-colors ${
                    !activeTag ? 'border-vault-accent text-vault-accent bg-vault-accent-soft' : 'border-vault-border text-vault-muted hover:text-vault-text'
                }`}
                >
                All
                </button>
